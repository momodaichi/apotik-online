...
<link rel="stylesheet" href="{{url()}}/adminFiles/css/bootstrap-spinner.css" type="text/css" />
    ...
    <div class="m-b-md">
			...
			<div class="col-sm-6 portlet ui-sortable">
			  <section class="panel panel-default portlet-item"  >
				...
				Prescription
				  <i id='rotatePres' class="fa fa-refresh btn btn-default" style="float:right;border-radius:26px; display: none"></i>
				</header>
				<div style="padding:15px">
					<img id='presImgId' src="{{url()}}/public/images/prescription/<?php echo $email.'/'.$path;?>" style="display: block;width: 70%; height:50%;margin-left:100px"/>
				</div>
			  </section>
			</div>
			<div class="col-sm-6 portlet ui-sortable" >
			 <form action="{{url()}}/admin/update-invoice" method="POST" >
			  <section class="panel panel-default portlet-item"  style="height:700px;width:600px">
				...
				<?php if($status==1)
                				{?>
				 <button style="float:right"id="add_field_button" class="btn btn-sm btn-default"><i class="fa fa-plus-square" style="padding-right:5px"></i>Add Medicine</button>
				 <?php }?>
				 <input type="text" id="shipping" name="shipping" class="form-control auto-input" style="width:400px" placeholder="Enter Shipping Cost For This Order" value=<?php echo $shipping; ?>>
				</header>
				<div class="list-group bg-white" style="display: block;width: 100%; height: 625px;">
					<section class="vbox">
					  <section class="scrollable padder">
					    ...
					    <div class="m-b-md">
						<div class="input_fields_wrap" style="padding:15px">
						    <div class="row">
						        <div class="col-lg-3">
						        <h5>Medicine Name</h5>
						    	   <input type="text"  id="autocomplete1" name="autocomplete1" class="form-control auto-input" style="width:200px" name="mytext[]" placeholder="Type Medicine name"
						    	...
						    	<div class="col-lg-3" style="width:100px;margin-left:80px" >
						    	   <h5 >Qty</h5>
						    	   <input type="number" min="1" value=<?php 
						    	   					...
						    	   					?> id="qty1" name="qty1" onChange="myFunction(this.id)" autocomplete="off"  class="form-control" placeholder="Qty." 	>
						    	</div>
						    	<div class="col-lg-3" style="width:120px;margin-left:-8px">
						    	   <h5 >Total Price</h5>
					    			
						    	   <input type="text" id="price1"  class="form-control"  placeholder="Total" value=<?php 
						    	   				if(!empty($items[0])){
						    	   ...
						    	   <input id="item_code1" name="item_code1" type="hidden" value=<?php 
						    	   			...
						    	   			?> >
						    	   <input type="hidden" id="pricee1" name="pricee1" class="form-control"  placeholder="Total" value=<?php echo(isset($items[0]['unit_price'])?$items[0]['unit_price']:'');?> >
						    	</div>
						    <div class='col-lg-3'  style='width:120px;margin-left:-8px' >
						    <h5>Discount</h5>
						    <input type='text' id='discount1' class='form-control' placeholder='Discount' value=<?php echo $discount[0] ?> disabled >
						    </div>
 						    </div>
						</div>
					   </section>
					...
					$(document).ready(function() {
					  ...
					  //  var itemP=JSON.parse(itemS);
					    ...
					    var todelete =[];
					    var dis= <?php if(!empty($discount[0]))
					    ...
					    $(document).on('focus','.auto-input',function(e){
						 	...
						 	$(this).autocomplete({
							    ...
							    onSelect: function (suggestion) {
							      ...
							      $("#discount"+id).val(suggestion.discount);
							      $("#itemS").val(id);
							    }
							});
						 });
				...
				<?php if($status==1)
				{?>
				<button style="float:right"id="add_field_button" class="btn btn-s-md btn-success">Verify</button>
				<?php }?>
			 </section>
			</form>
		     </div>
...
<script src="{{url()}}/adminFiles/js/jquery.autocomplete.js"></script>
<script src="{{url()}}/adminFiles/js/jquery.spinner.js"></script>
